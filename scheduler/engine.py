...
class Scheduler:
    ...
    def _should_run(self, schedule: str, last_run: Optional[datetime], now: datetime) -> Tuple[bool, str]:
        if not last_run:
        ...
        try:
            ...
            if elapsed >= duration:
                return True, f"Interval of {schedule} exceeded (elapsed: {str(elapsed).split('.')[0]})"
            
            remaining = duration - elapsed
            return False, f"Interval of {schedule} not met (remaining: {str(remaining).split('.')[0]})"
    ...
    def _run_internal_service(self, task: TaskConfig, template_override: Optional[str] = None):
